...
run:
	...
	@docker run --rm -it -p 8080:8080 	-p 8081:8081 -d --name webserv_test_env webserv_test
...
test_virtual_server:
	...
	@curl -H 'Host: default_server' localhost:8081
	@echo -e "\n\e[31m --->> virtual_server (port 8080)\e[m\n"
	@curl -H 'Host: virtual_server' localhost:8081

## limit the client body
test_limit_client_body:
	@curl -X POST --data "`cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 16 | head -n 3000 | sort | uniq`" localhost:8080/upload-validate/
...
test_fileupload:
	@curl https://miro.medium.com/max/1400/1\*kRv2XzBz0AXYXfAPJfkwZQ.png > 42.png
	@curl -v http://localhost:8080/upload/ -H "Content-type: image/png" --data-binary @42.png
